...
`Qt_VideoPlayer` 是一款基于 Qt 6.10.2、C++17 开发的简易音视频播放器，采用 **CMake** 构建系统，支持在 **Qt Creator** 与 **VS Code / Trae** 等主流 IDE 中无缝开发。它旨在提供简洁易用的图形界面，同时展示 Qt 框架与多媒体处理库的结合实践，实际运行界面如下：
...
### 代码质量与可维护性优化
- 冗余代码清理：移除未使用的成员变量（`volumeSlider`、`volumeWidget`、`m_isVideoPlaying`）、冗余头文件（`QTimer`、`<utility>`）和纯转发槽函数（`seekPosition`），减少代码噪音。
- 重复逻辑合并：将"上一首/下一首"两套完全对称的实现合并为 `switchSong(int direction)`；将 5 处重复的文件名获取封装为 `currentFileName()`；将两处各自实现的索引计算统一为 `nextIndex(int direction)`。
- 变量 const 化：支持的音视频格式列表设为 `const`，在构造初始化列表中一次性赋值，明确只读语义。
- 辅助函数提取：抽取 `msToTimeString()` 时间格式化、`linearToLogVolume()` 对数音量转换、`currentIndex()` 边界校验等小工具函数，职责单一、可读性强。
- CMake 现代化：采用 `qt_standard_project_setup()` 替代手动设置 `AUTOMOC/AUTOUIC/AUTORCC`，移除 Qt5 兼容死分支，配置文件从 84 行精简至约 40 行。
- 改进点：VideoPlayer.cpp 从 513 行精简至约 320 行（减少约 37%），逻辑分层更清晰，便于后续扩展新功能。
