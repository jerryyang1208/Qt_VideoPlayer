...
> *VS 2022 平台下的程序运行结果（此处由于系统默认设置黑色主题，因此生成窗口也是黑色，无需在意）*

目前项目支持在 Qt Creator 和 VS 2022 兼容使用，仅增添配置文件无需修改核心代码，整个构建系统的工作机制：

- Qt Creator/VS 2022 作为 IDE：提供编辑、调试界面
- CMake 作为构建系统：跨平台配置，一个 CMakeLists.txt 通用
- Ninja 作为构建工具：快速编译
- MSVC 作为编译器：生成 Windows 可执行文件
- Qt 作为框架：提供 GUI 和多媒体功能

双 IDE 开发优势：

- UI 设计：Qt Creator 的可视化设计器更便捷
- 深度调试：VS 2022 的诊断工具更强大
- 团队协作：支持不同 IDE 偏好的开发者协同工作
